...
void Valeev_Init(void)
{
    ...
    fac = (double *)malloc(MAXFAC * sizeof(double));
    fac[0] = 1.0;
    for(i = 1; i < MAXFAC; i++)
        fac[i] = fac[i-1] * i;

    df = (double *)malloc(2 * MAXFAC * sizeof(double));
    df[0] = 1.0;
    df[1] = 1.0;
    df[2] = 1.0;
    ...
    bc = (double **)malloc(MAXFAC * sizeof(double*));
    for(i = 0; i < MAXFAC; i++)
    {
        bc[i] = (double *)malloc(MAXFAC * sizeof(double));
        for(j = 0; j <= i; j++)
                  ...
                  int norm_flag)
{
    ...
    pfac = 2 * pow(M_PI, 2.5) * K1 * K2 / (gammap * gammaq
            * sqrt(gammap + gammaq));
