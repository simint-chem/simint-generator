...
int main(int argc, char ** argv)
{
    ...
    p_shells[0].alpha[0] =   1.;  p_shells[0].coef[0] =  1.;
    ...
    struct simint_multi_shellpair ps2_pair;
    simint_initialize_multi_shellpair(&pp_pair);
    simint_initialize_multi_shellpair(&ps2_pair);
    ...
    long scratch_mem = simint_ostei_workmem(1, 1);
    printf("simint_ostei_workmem returns %ld ", scratch_mem);
    double * work = SIMINT_ALLOC(scratch_mem);

    // Argument is the number of doubles
    double * targets = SIMINT_ALLOC(7*7*7*7*12 * sizeof(double));
    int ncomputed = 0;
    int ntotal = 0;

    // 3rd argument is the screening tolerance. Set to 0.0 to disable
    ncomputed = simint_compute_eri_deriv(1,&pp_pair, &ps2_pair, 0, work, targets+ntotal);
    printf("Computed %d  ppps derivative integrals\n", ncomputed);
    ntotal += ncomputed;
    ntotal = (2*pp_pair.am1+1)*(2*pp_pair.am2+1)*(2*ps2_pair.am1+1)*(2*ps2_pair.am2+1)*12;
    ...
    simint_free_multi_shellpair(&ps2_pair);
    SIMINT_FREE(targets);
    SIMINT_FREE(work);
    simint_finalize();
