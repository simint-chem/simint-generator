...
)
{
    ...
    const int ncart_j = 1; // ((j+1)*(j+2))/2;
    const int ncart_k = ((k+1)*(k+2))/2;
    const int ncart_l = 1; // ((l+1)*(l+2))/2;

    const int ncart_i_1 = ((i)*(i+1))/2;
    const int ncart_j_1 = 1; // ((j)*(j+1))/2;
    const int ncart_k_1 = ((k)*(k+1))/2;
    const int ncart_l_1 = 1; // ((l)*(l+1))/2;
        ...
        for (int idx_k = 0; idx_k < ncart_k; idx_k++)
        {
            ...
            if ((ri->ijk[dir] > 1) && (rk->ijk[dir]))
            {
                for (int n = 0; n < num_n; n++)
                {
                    ...
                    const SIMINT_DBLTYPE ival = SIMINT_DBLSET1(ri->ijk[dir]-1);
                    SIMINT_DBLTYPE tmp1 = SIMINT_FMADD(a_over_p, theta2[idx4], theta2[idx3]);
                    const SIMINT_DBLTYPE tmp2 = SIMINT_MUL(ival, one_over_2p);
                    output[outidx] = SIMINT_FMADD(tmp1, tmp2, output[outidx]);
                    ...
                    const SIMINT_DBLTYPE kval = SIMINT_DBLSET1(rk->ijk[dir]);
                    tmp1 = SIMINT_MUL(one_over_2pq, kval);
                    output[outidx] = SIMINT_FMADD(tmp1, theta4[idx7], output[outidx]);
                }
            }
            ...
            if (rk->ijk[dir] <= 1) 
            {
                if (!ri->ijk[dir])
                {
                    for(int n = 0; n < num_n; n++)
                    {
                        ...
                        output[outidx] = SIMINT_MUL(QC[dir], theta1[idx1]);
                        idx1 += ncart1;
                        output[outidx] = SIMINT_FMADD(aoq_PQ[dir], theta1[idx1], output[outidx]);
                ...
                if (!ri->ijk[dir])
                {
                    ...
                    for(int n = 0; n < num_n; n++)
                    {
                        ...
                        // if (rk->ijk[dir] > 1)
                        const SIMINT_DBLTYPE rk_tmp1 = SIMINT_FMADD(a_over_q, theta2[idx4], theta2[idx3]);
                        output[outidx] = SIMINT_FMADD(rk_tmp1, tmp2, output[outidx]);
