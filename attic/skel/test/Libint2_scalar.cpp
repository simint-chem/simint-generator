                                   ...
                                   double * integrals)
{
    ...
    TimerType ticks_pre_0, ticks_pre_1;
    ...
    for(int b = 0;         b < libint_P->nshell2; b++, ab++)
    {
        ...
        for(int d = 0;         d < libint_Q->nshell2; d++, cd++)
        {
            ...
            for(int j = jstart; j < jend; j++)
            {
                ...
                const double W[3] = { (libint_Q->alpha[j] * libint_Q->x[j] + libint_P->alpha[i] * libint_P->x[i]) * one_over_PQalpha_sum,
                                      (libint_Q->alpha[j] * libint_Q->y[j] + libint_P->alpha[i] * libint_P->y[i]) * one_over_PQalpha_sum,
                                      (libint_Q->alpha[j] * libint_Q->z[j] + libint_P->alpha[i] * libint_P->z[i]) * one_over_PQalpha_sum };

                const double oo2z = one_over_Qalpha * one_half;
                const double oo2e = one_over_Palpha * one_half;
                const double oo2ze = one_over_PQalpha_sum * one_half;
                const double roz = rho * one_over_Qalpha;
                const double roe = rho * one_over_Palpha;
                ...
                // scaling factor for the boys function
                ...
                // copy to the libint structure, but only if not doing (ss|ss)
                CLOCK(ticks_copy_0);
                if(M)
                {
                    ...
                    erival[nprim].oo2z[0] = oo2z;
                    erival[nprim].oo2e[0] = oo2e; 
                    erival[nprim].oo2ze[0] = oo2ze;
                    erival[nprim].roz[0] = roz; 
                    erival[nprim].roe[0] = roe; 
                }
