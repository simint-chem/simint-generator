...
int eri_sharedwork_X_X_X_X(struct simint_multi_shellpair const P,
                           struct simint_multi_shellpair const Q,
                           double * const restrict contwork,
                           double * const restrict INT__X_X_X_X)
{
    // Since this function can be called for X_X_X_X, s_X_s_X, X_s_s_X, and s_X_X_s,
    ...
    const int ncart_L1 = ((L1+1)*(L1+2))/2;
    const int ncart_L2 = ((L2+1)*(L2+2))/2;

    memset(INT__X_X_X_X, 0, P.nshell12 * Q.nshell12 * ncart1234 * sizeof(double));
    ...
    // for ( i j | (k+l) 0 )
    for(n2 = 1; n2 <= P.am2; n2++)
    for(n1 = 0; n1 <= (P.am2-n2); n1++)
        arr_stack_size += ncart_L2 * ( ((n2+1)*(n2+2))/2 ) * ( ((n1+P.am1+1)*(n1+P.am1+2))/2 );

    // for ( i j | k l )
    for(n2 = 1; n2 <= Q.am2; n2++)
    for(n1 = 0; n1 <= (Q.am2-n2); n1++)
        arr_stack_size += ncart12 * ( ((n2+1)*(n2+2))/2 ) * ( ((n1+Q.am1+1)*(n1+Q.am1+2))/2 );


    __m256d * arr_stack = (__m256d *)(ALLOC(arr_stack_size * sizeof(__m256d)));
    __m256d * prim_ptr[L1+1][L2+1];
    __m256d * prim_ptr4[P.am1+1][P.am2+1][Q.am1+1][Q.am2+1];
    ...
    for(n1 = 0; n1 <= (P.am2-n2); n1++)
    {
        prim_ptr4[n1+P.am1][n2][L2][0] = pos;
        pos += ncart_L2 * ( ((n2+1)*(n2+2))/2 ) * ( ((n1+P.am1+1)*(n1+P.am1+2))/2 );
    }

    for(n2 = 1; n2 <= Q.am2; n2++)
    for(n1 = 0; n1 <= (Q.am2-n2); n1++)
    {
        prim_ptr4[P.am1][P.am2][n1+Q.am1][n2] = pos;
        pos += ncart12 * ( ((n2+1)*(n2+2))/2 ) * ( ((n1+Q.am1+1)*(n1+Q.am1+2))/2 );
    }
    ...
    for(ab = 0; ab < P.nshell12; ++ab)
    {
        ...
        __m256d AB[3] = { _mm256_set1_pd(P.AB_x[ab]),
                          _mm256_set1_pd(P.AB_y[ab]),
                          _mm256_set1_pd(P.AB_z[ab]) };
        ...
        for(cdbatch = 0; cdbatch < Q.nbatch; ++cdbatch)
        {
            ...
            for(i = istart; i < iend; ++i)
            {
                ...
                double * restrict PRIM_PTR_INT__X_X_X_X = INT__X_X_X_X + abcd * ncart1234;
                ...
                for(j = jstart; j < jend; j += SIMINT_SIMD_LEN)
                {
                    ...
                    if((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd)
                    {
                        ...
                        if(iprimcd >= nprim_icd && ((icd+1) < nshellbatch))
                        {
                            ...
                            PRIM_PTR_INT__X_X_X_X += ncart1234;
                            cd_off++;
                        }
                        iprimcd++;
                    ...
                    tmp_CD_z.d[0] = Q.AB_z[cd+cd_off];

                    for(n = 1; n < SIMINT_SIMD_LEN; ++n)
                    {
                        tmp_CD_x.d[n] = Q.AB_x[cd+cd_off+shelloffsets[n]];
                        tmp_CD_y.d[n] = Q.AB_y[cd+cd_off+shelloffsets[n]];
                        tmp_CD_z.d[n] = Q.AB_z[cd+cd_off+shelloffsets[n]];
                    }


                    __m256d CD[3] = { tmp_CD_x.d_256,
                                      tmp_CD_y.d_256,
                                      tmp_CD_z.d_256 };

                    // now do horizontal recurrence
                    // 1.) Bra
                    for(n2 = 1; n2 <= P.am2; n2++)
                    for(n1 = 0; n1 <= (P.am2-n2); n1++)
                    {
                        ...
                        __m256d * theta1 = prim_ptr4[P.am1+n1+1][n2-1][L2][0];
                        __m256d * theta2 = prim_ptr4[P.am1+n1][n2-1][L2][0];
                        if(n2 == 1)
                        {
                            theta1 = prim_ptr[P.am1+n1+1][L2];
                            theta2 = prim_ptr[P.am1+n2][L2];
                        }

                        general_hrr_j(P.am1+n1, n2, Q.am1, Q.am2,
                                      AB,
                                      theta1, theta2,
                                      prim_ptr4[P.am1+n1][n2][L2][0]);
                    }
                    ...
                    PRIM_PTR_INT__X_X_X_X += lastoffset*ncart1234; 
                    cd_off += lastoffset;
...
int eri_X_X_X_X(struct simint_multi_shellpair const P,
                struct simint_multi_shellpair const Q,
                double * const restrict INT__X_X_X_X)
{
    return eri_sharedwork_X_X_X_X(P, Q, NULL, INT__X_X_X_X);
}
