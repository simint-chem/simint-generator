...
void general_vrr2_k(int i, int k, int N,
...
void general_vrr2_i(int i, int k, int N,
                    __m256d one_over_2p, __m256d one_over_2pq,
                    __m256d a_over_p,
                    __m256d aop_PQ[3],
                    __m256d PA[3],
                    __m256d * theta1, __m256d * theta2, __m256d * theta3,
                    __m256d * output)
{
    ...
    const int ncart = ncart_i * ncart_k;
    const int ncart1 = ncart_i_1 * ncart_k;
    const int ncart2 = ncart_i_2 * ncart_k;
    const int ncart3 = ncart_i_1 * ncart_k_1;

    // we are mainly interested in i-1, etc. But we also need to know
    // some info about k-1 
    const int arrstart_i = am_recur_map[i];
    const int arrstart_k = am_recur_map[k];
    struct RecurInfo const * const aminfo_i = &recurinfo_array[arrstart_i]; 
    ...
    for(int16_t idx_k = 0; idx_k < ncart_k; idx_k++)
    {
        ...
        {
            const int outidx = n * ncart + curidx;
            const int idx1 =     n*(ncart1) + ri->idx[dir][0] * ncart_k + idx_k;
            const int idx2 = (n+1)*(ncart1) + ri->idx[dir][0] * ncart_k + idx_k;

            output[outidx] = PA[dir]*theta1[idx1] + aop_PQ[dir]*theta1[idx2];

            if(rk->idx[dir][0] > -1) // -1 signifies that it doesn't exist
            {
                const int idx5 = (n+1)*(ncart3) + ri->idx[dir][0] * ncart_k_1 + rk->idx[dir][0];
                const __m256d k1 = _mm256_set1_pd(rk->ijk[dir]);
                output[outidx] += k1 * one_over_2pq * theta3[idx5];
            }

            if(ri->ijk[dir] > 1)
            {
                const int idx3 =     n*(ncart2) + ri->idx[dir][1] * ncart_k + idx_k;
                const int idx4 = (n+1)*(ncart2) + ri->idx[dir][1] * ncart_k + idx_k;
                const __m256d i1 = _mm256_set1_pd(ri->ijk[dir]-1);
                output[outidx] += one_over_2p * i1 * (theta2[idx3] + a_over_p * theta2[idx4]);
            }
        }
