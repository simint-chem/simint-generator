                        ...
                        double * const integralwork2)
{
    ...
    double * const restrict integralwork1_short = integralwork1;
    double * const restrict integralwork1_long = integralwork1 + SIMD_ROUND_DBL(nprim1234);

    double * const restrict integralwork2_short = integralwork2;
    double * const restrict integralwork2_long = integralwork2 + SIMD_ROUND_DBL(nprim1234);
        ...
        for(cd = 0; cd < Q.nshell12; cd++)
        {
            for(i = abstart; i < abend; ++i)
            {
                ...
                for(j = cdstart; j < cdend; ++j)
                {
                    ...
                    // store the paremeter to the boys function in integralwork1
                    const double Fparam = R2 * PQalpha_mul/PQalpha_sum;


                    if(Fparam < BOYS_GRID_MAXX)
                    {
                        integralwork1_short[idx_short] = Fparam;
                        integralwork2_short[idx_short] = pfac * P.prefac[i] * Q.prefac[j];
                        idx_short++;
                    }
                    else
                    {
                        integralwork1_long[idx_long] = Fparam;
                        integralwork2_long[idx_long] = pfac * P.prefac[i] * Q.prefac[j];
                        idx_long++;
                    }
                }
            }
